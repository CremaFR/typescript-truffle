...
export function fireAsyncSuccess<A = {}>( action: flux.PromiseAction<A>, res?: any ): Observable<any> {
    if( action.success ) action.success();
    return Observable.of( res ) || Observable.empty();
}
...
export function epicAsync<A = {}, Er = { [key: string]: any }>( action: flux.PromiseAction<A, Er>, obs: Observable<any>, toEr: ( a: any ) => Er ): Observable<any> {
    return obs.mergeMap( c => {
        return fireAsyncSuccess( action, c );
    } ).catch( ( err, _ ) => {
